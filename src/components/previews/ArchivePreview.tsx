    ...
    <div className="w-full h-full flex flex-col bg-[var(--bg-card)] text-[var(--text-main)] overflow-hidden select-none">
      ...
      <div className="px-3.5 py-1.5 bg-[var(--bg-surface)] border-b border-[var(--border-app)] flex items-center justify-between gap-3 text-xs shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-bold uppercase tracking-wider text-[10px] text-[var(--text-muted)]">Archive Catalog</span>
        </div>

        <span className="px-2 py-0.5 rounded-md bg-amber-500/10 border border-amber-500/30 text-[10px] text-amber-400 font-mono uppercase font-bold tracking-wider shrink-0">
          {file.extension}
        </span>
      </div>

      {/* Hero Stats */}
      <div className="p-3 bg-[var(--bg-surface)]/50 border-b border-[var(--border-app)] grid grid-cols-3 gap-2 text-center text-xs shrink-0">
        <div className="p-2 rounded-xl bg-[var(--button-bg)] border border-[var(--border-app)] flex flex-col items-center">
          <span className="text-[10px] text-[var(--text-subtle)] uppercase font-medium flex items-center gap-1">
            <HardDrive className="w-3 h-3 text-sky-400" /> Size
          </span>
          <span className="font-bold text-[var(--text-main)] mt-0.5">{formatBytes(file.sizeBytes)}</span>
        </div>
        ...
        <div className="p-2 rounded-xl bg-[var(--button-bg)] border border-[var(--border-app)] flex flex-col items-center">
          ...
          <span className="font-bold text-[var(--text-main)] mt-0.5">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {result?.isSupported && result.entries.length > 5 && (
          <div className="px-4 py-2 border-b border-[var(--border-app)] bg-[var(--bg-surface)]">
            <input
              type="text"
              placeholder="Search archive contents..."
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full px-3 py-1.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-app)] text-xs text-[var(--text-main)] placeholder-[var(--text-subtle)] focus:outline-none focus:border-amber-500/50"
            />
          </div>
        )}

        <div className="flex-1 overflow-y-auto divide-y divide-[var(--border-subtle)] p-2">
          {loading ? (
            ...
            filteredEntries.length === 0 ? (
              ...
              filteredEntries.map((entry, idx) => (
                <div
                  key={idx}
                  className="px-3 py-1.5 flex items-center justify-between gap-3 text-xs hover:bg-[var(--button-bg)] rounded-lg transition-colors"
                >
                  <div className="flex items-center gap-2.5 min-w-0 flex-1">
                    {entry.isDir ? (
                      <Folder className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                    ) : (
                      <File className="w-3.5 h-3.5 text-[var(--text-subtle)] shrink-0" />
                    )}
                    <span className="truncate text-[var(--text-main)] font-mono text-[11px]">
                      {entry.name}
                    </span>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center h-48 text-center p-6 text-[var(--text-muted)]">
              <Archive className="w-12 h-12 text-amber-400/50 mb-3" />
              <p className="text-xs font-semibold text-[var(--text-main)]">Compressed Archive Container</p>
